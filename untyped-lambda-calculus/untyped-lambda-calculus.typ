...
#slide[
  ...
  在 $(lambda s. M)$ 这个函数中，$s$ 就是 $(lambda s. M)$ 自身，我们想让 $s = (lambda s. M)$。
]
...
#slide[
  ...
  我们想让 $s = (lambda s. M)$，这样的话，在 $M$ 中，我们就有 $s s = M$。
...
#slide[
  ...
  #columns(2)[
    ...
    - 完备性：所有真命题都能被证明；
    - 一致性：不可能导出矛盾；
    - 可判定性：应该有一个算法，来确定每一个形式化的命题是真命题还是假命题。
  ]
]
...
#slide[
  ...
  并证明了它们刻画了同一类函数，这种惊人的一致性促成了邱奇–图灵论题：
  #align(center)[
...
#slide[
  ...
  在我们的形式化中，#lc;的操作语义是用“替换”来实现的，这是一种纯语法的变换，也很难被#import "@preview/polylux:0.4.0": later, only, slide, uncover
#import "@preview/simplebnf:0.2.0": Or, Prod, bnf
#import "@preview/tdtr:0.5.5": tidy-tree-graph
#import "@preview/ctheorems:1.1.3": thmrules
#import "@preview/curryst:0.6.0": prooftree, rule, rule-set
